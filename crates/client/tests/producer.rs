...
mod send {
    ...
    pub async fn produce_acks() {
        ...
        let record = FutureRecord::new(topic.name(), vec![], vec![]);
    ...
    #[test_log::test(tokio::test)]
    pub async fn full_batches_are_not_waiting_for_linger_timeout() {
        let (_kafka_cluster, cluster, topic) = create_single_node_with_single_topic().await;
        ...
        let ctx = &mut Context::from_waker(Waker::noop());
        let record_small = FutureRecord::new(topic.name(), vec![], [0_u8; 20]);
        let record_large = FutureRecord::new(topic.name(), vec![], [0_u8; 500]);

        let mut result_1 = pin!(producer.send(record_small.clone()));
        tokio::time::sleep(Duration::from_millis(100)).await;
        let mut result_2 = pin!(producer.send(record_large.clone()));
        assert!(matches!(result_1.as_mut().poll(ctx), Poll::Pending));
        assert!(matches!(result_2.as_mut().poll(ctx), Poll::Pending));
        let mut result_3 = pin!(producer.send(record_large));
        tokio::time::sleep(Duration::from_millis(100)).await;
        assert!(matches!(result_1.as_mut().poll(ctx), Poll::Ready(_)));
        assert!(matches!(result_2.as_mut().poll(ctx), Poll::Ready(_)));
        assert!(matches!(result_3.as_mut().poll(ctx), Poll::Ready(_)));
