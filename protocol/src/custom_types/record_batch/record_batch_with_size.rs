...
impl FromBytes for RecordBatchWithSize {
    fn deserialize(buf: &mut Bytes, is_flexible_version: bool) -> Self {
        let size: i32 = FromBytes::deserialize(buf, is_flexible_version);
        let mut slice = buf.split_to(size as usize);
        let mut batches = vec![];
        while !slice.is_empty() {
            batches.push(FromBytes::deserialize(&mut slice, is_flexible_version));
        }
...
impl ToBytes for RecordBatchWithSize {
    fn serialize(&self, buf: &mut BytesMut, is_flexible_version: bool) {
        ...
        for batch in &self.batches {
            batch.serialize(&mut buffer, is_flexible_version);
        }
        let size: i32 = buffer.len() as i32;
        size.serialize(buf, is_flexible_version);
        buf.extend(buffer);
    }
}
