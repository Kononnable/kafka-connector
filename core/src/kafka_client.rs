...
impl BrokerClient{
    ...
    async fn get_supported_api_versions(&mut self){
        ...
        let header = HeaderRequest::new(ApiNumbers::ApiVersions as i16,0, self.last_correlation+1, "".to_owned());
        ...
        for api_key in response.api_keys{
            self.supported_versions.insert(api_key.api_key,(api_key.min_version,api_key.max_version));
        }
